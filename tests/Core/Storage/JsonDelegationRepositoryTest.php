...
describe('JsonDelegationRepository', function (): void {
    ...
    describe('Coverage Edge Cases', function (): void {
        ...
        test('handles file read failure in single file mode gracefully', function (): void {
            ...
            // Create file that exists but will be difficult to read
            // This test covers the file_get_contents === false path
            $filePath = $this->tempDir.'/delegations/delegations.json';
            mkdir(dirname($filePath), 0o755, true);

            // Create symlink to non-existent file to simulate read failure scenario
            $targetFile = $this->tempDir.'/non-existent-target.json';
            symlink($targetFile, $filePath);
            ...
            // Cleanup
            unlink($filePath);
        })->skip('File permission tests are environment-dependent');
