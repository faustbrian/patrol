...
describe('RestfulRuleMatcher', function (): void {
    ...
    describe('Edge Cases', function (): void {
        ...
        test('handles PCRE backtrack limit error in preg_replace', function (): void {
            // This is a more explicit test for the null check on line 214
            // It ensures the code path where preg_replace returns null is executed
            $originalLimit = ini_get('pcre.backtrack_limit');

            try {
                // Set a very low backtrack limit to force preg_replace to fail
                ini_set('pcre.backtrack_limit', '1');
                ...
                // When preg_replace fails, it returns null, and line 214 sets $pattern = ''
                // This results in regex '#^$#' which only matches empty strings
                $result = $this->matcher->matches($rule, $subject, $resource, $action);

                // We don't care about the specific result, just that no exception is thrown
                // and line 214 is executed to set $pattern = '' when null is returned
