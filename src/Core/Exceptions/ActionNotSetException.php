...
/**
 * Thrown when attempting to build a policy rule without setting the required action.
 *
 * This exception indicates a programming error in the policy builder workflow where
 * the action component was not specified before finalizing the rule. Actions define
 * what operation is being authorized (e.g., "read", "write", "delete") and are
 * mandatory for all policy rules. This validation ensures that incomplete rules
 * cannot be created, preventing runtime authorization failures.
 *
 * Common causes:
 * - Calling addRule() without first calling action()
...
{
    /**
     * Create a new exception instance for missing action configuration.
     *
    ...
    {
        $message = 'Action must be set before adding a rule';
     ...
     * identifiers that were being configured when the action was found to be missing.
