...
describe('StorageFactory', function (): void {
    ...
    describe('Default Path Handling', function (): void {
        test('getDefaultPath attempts to use storage_path when function exists', function (): void {
            // Arrange
            ...
            // Act & Assert
            // storage_path() exists in Laravel but app() isn't bootstrapped in unit tests
            // This proves line 176 is executed (the storage_path call)
            expect(fn (): mixed => $method->invoke($factory))
                ->toThrow(Error::class, 'storagePath');
        });
    });
});
