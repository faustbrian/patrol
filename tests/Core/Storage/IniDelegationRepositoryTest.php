...
describe('IniDelegationRepository', function (): void {
    ...
    describe('Regressions', function (): void {
        ...
        test('handles buildDirectoryPath with non-null version', function (): void {
            // Arrange - This tests line 423 where version is not null
            ...
            // Assert - File should be in versioned directory (line 423 executed)
            $filePath = $this->tempDir.'/delegations/3.0.0/del-versioned-path.ini';
            expect($filePath)->toBeFile();
        });
        ...
        test('handles Exception during INI parsing in loadSingleFile', function (): void {
            ...
            // This tests lines 285-286 (catch Exception block)
            $filePath = $this->tempDir.'/delegations/delegations.ini';
            mkdir(dirname($filePath), 0o755, true);

            // Create content that causes parse_ini_string to throw
            // Severely malformed INI syntax that triggers Exception
...
INI;
        ...
        test('handles glob returning false in loadMultipleFiles', function (): void {
            // Arrange - This tests line 307 where glob() could return false
            // However, glob() only returns false on error (not for empty results)
            // We can't easily simulate this without mocking, but we can test
            // the path by using a pattern that might cause issues

            // Create a repository pointing to a path with special characters
            // that might cause glob to fail in some systems
            $specialPath = $this->tempDir.'/delegations[test]';

            // Don't create the directory - glob on non-existent path returns empty array or false
            $repository = new IniDelegationRepository(
                basePath: $this->tempDir,
                fileMode: FileMode::Multiple,
                versioningEnabled: false,
            ...
            // Act - loadMultipleFiles when directory doesn't exist
            $found = $repository->findById('any-id');

            // Assert - Should return null when directory doesn't exist (line 301 check)
            expect($found)->toBeNull();
        });
    });
});
