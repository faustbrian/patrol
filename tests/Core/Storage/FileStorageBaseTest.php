...
describe('FileStorageBase', function (): void {
    ...
    describe('Edge Cases', function (): void {
        ...
        test('detectLatestVersion returns null when scandir fails due to permissions', function (): void {
            // Arrange
            $typeDir = $this->tempDir.'/unreadable';
            mkdir($typeDir, 0o755, true);

            // Remove read permission to trigger scandir failure
            chmod($typeDir, 0o000);
            ...
            try {
                // Act
                // scandir will return false when directory cannot be read
                // The @ operator suppresses the PHP warning that is expected
                $result = $storage->exposedDetectLatestVersion($typeDir);

                // Assert
                expect($result)->toBeNull();
            } finally {
                // Cleanup - restore permissions before test teardown
                chmod($typeDir, 0o755);
            }
        })->skip(fn (): bool => \DIRECTORY_SEPARATOR === '\\', 'Permission tests unreliable on Windows');
    });
});
